...
每次 run 把记忆正文与标题索引放入系统背景：文件不超过注入预算时直接放全文（省掉「读不全 → 再 memory_get」的往返），超过预算时退化为 `# 核心记忆` 的预算内内容。核心预算为 `min(32000, max(4000, contextWindow / 16))` 个字符；模型窗口未知时按 128K 计算。标题索引带行号范围（如 `## 设备  [L12-45]`），便于定点 `memory_get` 或行区间替换。既超出预算又没有 `# 核心记忆` 标题时不注入正文，只给索引；其余内容由 `memory_get` 按行分页或按文本检索，单次最多返回 32000 字符。

`memory_write` 的四种模式：`upsert_section`（按标题整章替换，命中同名章节就替换该章节内容并保留原标题层级，未命中在文末新增，匹配时 `#` 层级不参与比较）、`replace_range`（1 起闭区间行替换）、`append`（文末追加新章节）、`clear`（清空）。`mode` 与 `revision` 都可以省略：省略 `mode` 时按字段推断（heading → upsert_section、start_line/end_line → replace_range、content → append），省略 `revision` 表示按写入时的最新内容执行、不做并发检查；只有传了 `revision` 且与当前内容不一致时才返回 `MEMORY_CONFLICT`。`upsert_section` 若会连带删掉被替换章节下的子章节，返回 `MEMORY_SECTION_NESTED` 而不是静默丢失。单次模型生成内容最多 8000 字符，设置页的用户手动编辑不受此单次工具限制。关闭记忆不会删除文件，后续 run 不再注入或暴露工具；已开始的 run 在每次执行记忆工具前也会重新检查开关。
