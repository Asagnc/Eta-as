...
设置首页使用按功能分类的蓝、绿、橙、黄图标，图标直接着色且不添加底板；不可用项保留主题禁用色。首页行布局独立于通用 Preference 的固定最小高度，普通行、开关行和选择行采用一致的最小行高；长标题、摘要和大字体可自然撑高。图标使用统一占位并对满幅轮廓作光学校正，正文与细分割线共享起点。卡片边距、圆角与行内间距集中在首页样式中，开关和选择菜单继续使用 Miuix；设置首页浅色默认背景与白色卡片形成层次，Monet 和深色配置沿用 App 主题。
