 ...
 * 迁移链的结构断言。
 *
 * 这个测试**故意不依赖 Robolectric**：`StaMigrations` 只用到 `androidx.room.migration.Migration`，
 * 所以能在本地 aarch64 上跑。而真正的迁移行为测试（`StaDatabaseMigrationTest`）需要 Robolectric，
 * 本地跑不了、只能等 CI——"版本号涨了但迁移没接上"这类错误以前就只能在 CI 里暴露。
 *
 * 把结构约束挪到本地可跑的这一层，是这次 CI 失败换来的教训。
 */
class StaMigrationsChainTest {

    private val migrations = StaMigrations.ALL
    ...
    fun `迁移链连续覆盖从最老支持版本到当前版本`() {
        ...
        assertEquals(
            ...
            StaMigrations.MIN_SUPPORTED_VERSION,
            sorted.first().startVersion,
        )
        var expected = StaMigrations.MIN_SUPPORTED_VERSION
    ...
    fun `当前版本与最老支持版本的常量互相自洽`() {
        assertTrue(
            ...
            StaMigrations.MIN_SUPPORTED_VERSION < StaMigrations.CURRENT_VERSION,
        )
    }
}
