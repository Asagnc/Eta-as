...
@Config(sdk = [36])
class StaDatabaseMigrationTest {
    @Test
    fun migration6To21PreservesDataAndMovesCompleteConversationContext() {
        ...
        val migration16To17WithMcpData = Migration(16, 17) { database ->
            StaMigrations.MIGRATION_16_17.migrate(database)
            database.execSQL(
        ...
        // 从 StaMigrations.ALL 取链，只把 16->17 换成带测试数据的版本：
        // 这样以后新增迁移不必再改这个测试，也不会再出现"生产加了、测试清单没加"的漂移。
        val migrations = StaMigrations.ALL
            .filterNot { it.startVersion == 16 && it.endVersion == 17 }
            .plus(migration16To17WithMcpData)
        val database = Room.databaseBuilder(context, StaDatabase::class.java, databaseName)
