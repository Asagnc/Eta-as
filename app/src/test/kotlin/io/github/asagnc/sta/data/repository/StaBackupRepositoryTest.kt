...
import io.github.asagnc.sta.data.model.ModelSource
import io.github.asagnc.sta.data.model.withApiKey
...
import io.github.asagnc.sta.agent.roleplay.RoleplayReplyRevision
...
class StaBackupRepositoryTest {
    ...
    fun exportAndImportRestoresProvidersConversationsAndMemory() = runBlocking {
        ...
        StaDatabase.get(context).conversationDao().replaceAll(
        ...
        val exported = StaBackupRepository.export(context, output)
        ...
        val imported = StaBackupRepository.import(
            context,
        ...
        assertEquals(
            "保留这条消息",
            StaDatabase.get(context).conversationDao().messages().single().content,
        )
    ...
    @Test(expected = StaBackupException::class)
    fun rejectsUnknownBackupFormatBeforeChangingData(): Unit = runBlocking {
        StaBackupRepository.inspect(
    ...
    fun roleplayBackupRestoresCardsPersonaAvatarMemoryAndLargeSessionMetadata() = runBlocking {
        ...
        StaDatabase.get(context).conversationDao().insertConversations(listOf(
        ...
        StaDatabase.get(context).conversationDao().insertMessages(listOf(
            ConversationMessageEntity("role-message", "role-session", 0, "assistant", "版本".repeat(40_000)),
        ))
        val output = ByteArrayOutputStream()
        assertEquals(1, StaBackupRepository.export(context, output).characterCount)
        ...
        StaBackupRepository.import(context, ByteArrayInputStream(output.toByteArray()))
        ...
        val row = StaDatabase.get(context).conversationDao().conversationEntities().single()
    ...
    fun memoryWriteFailureRollsBackDatabaseAndEarlierMemoryFiles() = runBlocking {
        ...
        val database = StaDatabase.get(context)
        ...
        val document = StaBackupDocument(exportedAt = 0, memoryMd = "新的现实记忆",
        ...
        try {
            var failed = false
            try {
                StaBackupRepository.import(context, ByteArrayInputStream(Json.encodeToString(document).toByteArray()))
    ...
    fun invalidRevisionBackupIsRejectedBeforeExistingConversationsAreChanged() = runBlocking {
        val dao = StaDatabase.get(context).conversationDao()
        ...
        invalidStates.forEach { revisions ->
            val document = StaBackupDocument(exportedAt = 0,
