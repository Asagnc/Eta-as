...
import io.github.asagnc.sta.agent.model.AgentToolBatchRecovery
import io.github.asagnc.sta.data.db.StaDatabase
import io.github.asagnc.sta.data.db.RuntimeInFlightEventEntity
import io.github.asagnc.sta.data.db.RuntimeInFlightRunEntity
...
internal object AgentRunCheckpointStore {
    ...
    fun start(
        ...
        runBlocking(Dispatchers.IO) {
            StaDatabase.get(context.applicationContext).runtimeRunDao().replaceInFlightRun(
    ...
    fun append(
        ...
        runBlocking(Dispatchers.IO) {
            StaDatabase.get(context.applicationContext).runtimeRunDao().appendInFlightEvent(
    ...
    fun list(context: Context): List<Checkpoint> =
        runBlocking(Dispatchers.IO) {
            StaDatabase.get(context.applicationContext)
                .runtimeRunDao()
                .inFlightRuns()
                .asSequence()
                .map { stored ->
    ...
    fun saveTranscript(context: Context, runId: String, transcript: List<AgentModelClient.ConversationMessage>) {
        runBlocking(Dispatchers.IO) {
            StaDatabase.get(context.applicationContext).runtimeRunDao()
    ...
    fun saveContext(context: Context, runId: String, snapshot: AgentContextSnapshot) {
        runBlocking(Dispatchers.IO) {
            StaDatabase.get(context.applicationContext).runtimeRunDao().updateContextSnapshot(runId, snapshot.encode())
        }
    }
