        ...
        messages.put(
            systemMessage(
                (if (roleplayContext == null) {
                    "你是 Sta。用户询问你的身份时说明你是 Sta；"
                } else {
                    "本会话通过 Sta Agent Runtime 运行角色人格。按后续人物设定交流；现实工具操作仍由 Sta 完成。" +
                    ...
                    (if (config.terminalTools) {
                        "读多个文件一律用 read_files 一次提交：它按路径列表读、预算在文件之间共享、" +
                            "每段带真实行号与续读位置，是“一次看多个”的现成动作。" +
                            "不要用 run_code 写循环逐个读文件——那是自己搭轮子，也看不出批量意图；" +
                            "一次只读一个会让步数随文件数线性增长，而步数越多，最终用掉多少步越不可预测。" +
                            "run_code 只做计算、过滤、统计、批量改写这类需要在沙箱里连续处理的工作。" +
                            "两者都不要改用 terminal 的 cat/grep/sed 绕过——同样是整段原文进上下文。" +
                            "开局会在系统提示里给出工作区清单（目录、文件名、行数、顶层类型名）：据此直接定位，" +
                                "一次把相关文件读完，不要在轮次之间逐个摸索。" +
                    ...
                    "屏幕观察与 GUI 操作前会确认 Sta 无障碍服务；只有系统保护后端可用时才会请求有限重绑。" +
                ...
                systemMessage(
                    ...
                    "带副作用的调用仍按提交顺序逐个执行。" +
                        ...
                        (if (rootAvailable) {
                            ...
                            "当前终端只支持 identity=user，以 Sta 的 App UID 执行；Linux 内模拟 root 不授予 Android 特权。用户未指定环境的命令使用 terminal 的 environment=android、action=open_and_exec；"
                        }) +
                        ...
                        "Sta 已内置终端，不要回答‘没有终端应用’或要求另装终端 App。" +
