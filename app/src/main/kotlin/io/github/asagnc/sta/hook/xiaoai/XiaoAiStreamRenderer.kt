package io.github.asagnc.sta.hook.xiaoai
...
internal class XiaoAiStreamRenderer(
    ...
    fun onEvent(event: AgentEvent) {
        when (event) {
            ...
            is AgentEvent.ProviderResponseStarted -> render(
                text(R.string.injected_reasoning, "Sta is reasoning…"),
            )
            ...
            is AgentEvent.ToolStarted -> {
                if (synchronized(streamedText) { streamedText.isEmpty() }) {
                    render(text(R.string.injected_executing, "Sta is working…"))
                }
            }
    ...
    fun complete(content: String) {
        ...
        val finalText = content.trim().ifBlank {
            text(R.string.injected_completed, "Sta completed this task")
        }
    ...
    fun fail() {
        ...
        render(
            text(
                ...
                "Sta could not complete the task. Try again later",
            ),
            immediate = true,
        )
    }
    ...
    private fun text(resourceId: Int, englishFallback: String): String =
        StaInjectedStrings.get(context, resourceId, englishFallback)
    ...
    private fun speak(text: String) {
        try {
            ...
            logger.warnThrottled("xiaoai_tts_failed") {
                "超级小爱 Sta 结果朗读失败: type=${exception.safeLogType()}"
            }
        }
    }
