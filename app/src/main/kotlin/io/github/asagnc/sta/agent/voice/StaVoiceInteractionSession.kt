 ...
 * 系统数字助理的入口桥接。
 *
 * 系统会为本类创建 TYPE_VOICE_INTERACTION 窗口，但 Sta 的实际界面由自己的
 * TYPE_APPLICATION_OVERLAY 窗口承载，避免把厂商助手动画和输入层级绑定到系统会话窗口。
 */
internal class StaVoiceInteractionSession(context: Context) : VoiceInteractionSession(context) {
    ...
    override fun onShow(args: Bundle?, showFlags: Int) {
        ...
        StaAssistantOverlayService.show(context)
    }

    override fun onBackPressed() {
        StaAssistantOverlayService.dismiss(context)
        hide()
    }
    ...
    internal companion object {
        private const val ACTION_HIDE_FOR_FOREGROUND_OPERATION =
            "io.github.asagnc.sta.agent.voice.HIDE_FOR_FOREGROUND_OPERATION"
