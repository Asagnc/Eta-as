...
import io.github.asagnc.sta.data.db.ConversationContextCheckpointEntity
import io.github.asagnc.sta.data.db.ConversationEntity
import io.github.asagnc.sta.data.db.ConversationMessageEntity
import io.github.asagnc.sta.data.db.ConversationStateEntity
import io.github.asagnc.sta.data.db.StaDatabase
import io.github.asagnc.sta.data.db.ProviderEntity
import io.github.asagnc.sta.data.db.ProviderModelEntity
import io.github.asagnc.sta.data.db.ProviderWithModelsSeed
...
/** Sta 用户数据备份；旧备份没有角色字段时保留设备上的角色库。 */
@Serializable
internal data class StaBackupDocument(
    val format: String = FORMAT,
    val schemaVersion: Int = SCHEMA_VERSION,
    val exportedAt: Long,
    val providers: List<StaBackupProvider> = emptyList(),
...
@Serializable
internal data class StaBackupProvider(
    val provider: ProviderEntity,
    val models: List<ProviderModelEntity> = emptyList(),
)

internal data class StaBackupSummary(
...
internal class StaBackupException(message: String, cause: Throwable? = null) :
    IllegalArgumentException(message, cause)

internal object StaBackupRepository {
    ...
    suspend fun export(context: Context, output: OutputStream): StaBackupSummary =
        withContext(Dispatchers.IO) {
            ...
            if (bytes.size > MAX_BACKUP_BYTES) {
                throw StaBackupException("备份文件超过 64 MiB 限制")
            }
            output.write(bytes)
            output.flush()
            document.summary()
    ...
    suspend fun import(context: Context, input: InputStream): StaBackupSummary =
        withContext(Dispatchers.IO) {
            val document = readDocument(input)
            validate(document)
            val appContext = context.applicationContext
            val database = StaDatabase.get(appContext)
    ...
    suspend fun inspect(input: InputStream): StaBackupSummary = withContext(Dispatchers.IO) {
        val document = readDocument(input)
        validate(document)
        document.summary()
    }

    private suspend fun snapshot(context: Context): StaBackupDocument {
        val appContext = context.applicationContext
        val database = StaDatabase.get(appContext)
    ...
    private fun readDocument(input: InputStream): StaBackupDocument {
        val bytes = input.readBytesLimited(MAX_BACKUP_BYTES)
        if (bytes.isEmpty()) throw StaBackupException("备份文件为空")
        return runCatching {
            json.decodeFromString<StaBackupDocument>(bytes.toString(Charsets.UTF_8))
        }.getOrElse { failure ->
            throw StaBackupException("备份文件格式无效", failure)
        }
    }

    private fun validate(document: StaBackupDocument) {
        if (document.format != StaBackupDocument.FORMAT) {
            throw StaBackupException("这不是 Sta 备份文件")
        }
        if (document.schemaVersion !in 1..StaBackupDocument.SCHEMA_VERSION) {
            throw StaBackupException("不支持的 Sta 备份版本：${document.schemaVersion}")
        }
        ...
        if (providerIds.size != providerIds.toSet().size || providerIds.any(String::isBlank)) {
            throw StaBackupException("备份中的模型提供商存在重复或无效 ID")
        }
        ...
        if (document.selectedModelId != null && selectedModel == null) {
            throw StaBackupException("备份中的当前模型不存在")
        }
        if (selectedModel != null && selectedModel.second != document.selectedProviderId) {
            throw StaBackupException("备份中的当前模型与提供商不匹配")
        }
        ...
        try {
            ...
            throw StaBackupException("备份中的角色数据无效", failure)
        }
    }

    private fun StaBackupDocument.summary(): StaBackupSummary = StaBackupSummary(
