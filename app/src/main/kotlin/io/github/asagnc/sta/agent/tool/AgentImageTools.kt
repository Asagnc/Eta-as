...
import io.github.asagnc.sta.agent.device.BoundedFileCopy
import io.github.asagnc.sta.agent.device.RootAccess
import io.github.asagnc.sta.agent.media.AgentImageCodec
import io.github.asagnc.sta.agent.media.MAX_AGENT_IMAGE_BYTES
import io.github.asagnc.sta.agent.model.AgentFileVisionToolCatalog
...
internal class AgentImageTools(
    ...
    private fun readOne(rawSource: String): AgentModelClient.ToolResult {
        ...
        return try {
            ...
            if (!staged) {
                if (!rootAvailable()) {
                    return sensitive(error("IMAGE_ACCESS_DENIED", "Sta 无法读取此图片；请先通过文件选择器导入或授予读取权限"))
                }
    ...
    private fun copyFailure(result: BoundedRootCommandExecutor.Result): String = when (result.exitCode) {
        ...
        23 -> error("IMAGE_STAGE_FAILED", "Root 无法将图片复制到 Sta 临时缓存")
