...
/**
 * 结构尺寸：**承载内容的容器**本身多大——控件高度、行的最小高度、头像边长。
 *
 * 与相邻族的边界：
 * - [StaSpacing]：两个元素之间留多少（padding、gap、Arrangement）
 * - [StaIconSize]：图标与装饰点本身多大
 * - [StaSize]：装着内容的容器多大
 * - [StaRadius] / [StaStroke]：圆角与描边
 *
 * 命名用「s + 实际数值」（[s28]），与 [StaSpacing] / [StaIconSize] 的尺码名不同。
 * 理由：那两个族是**标量刻度**，值落在 4dp 网格上，尺码名能表达「大中小」；
 * 结构尺寸不是刻度而是**布局约束**——改造前控件高度挤在 24~88 之间的 17 个值上
 * （28/30/32/34/36/38/40/42/44 连续无阶梯），套尺码名会把不同的值压成同一个名字，
 * 丢掉「这里是 30 不是 28」这个信息。数值名不漂移，也能全局替换。
 *
 * 数值保持改造前的实际值，**不做网格吸附**：这些是布局约束不是节奏刻度。
 * `minHeight = 30.dp` 的操作按钮改成 32 会挤掉一行；`widthIn(max = 360.dp)` 的
 * 面板改成 352 会让三栏布局换行。吸附 = 改设计。
 *
 * 改造前实测的**同语义分裂**（收敛只需 ≤2dp 位移，等看到渲染结果再决定）：
 * - 消息操作按钮在 [ChatMessageItem] 里 [s28] 与 [s30] 并存
 * - 输入栏与附件按钮 [s40] 与 [s42] 并存
 * - 列表行最小高度：ConversationPanePanel 取 [s48]，SettingsItemStyle 取 [s52]
 * - 附件条目 [s36] 与 [s38] 并存、模型控件行 [s34] 紧邻 [s32]
 */
object StaSize {

    /** 0：显式清零。比写 `0.dp` 更能表达「这里不要尺寸」。 */
    val none = 0.dp

    // ── 控件尺寸：按钮、输入框、行的最小高度与边长 ──

    /** 24：紧凑行内元素、折叠态行高、底部渐隐高度。 */
    val s24 = 24.dp

    /** 28：小图标按钮、内联进度指示器。 */
    val s28 = 28.dp

    /** 30：消息操作按钮。与 [s28] 同语义（改造前并存）。 */
    val s30 = 30.dp
    ...
    /** 40：输入栏按钮、主操作、工具卡最小高度。 */
    val s40 = 40.dp

    /** 42：附件条目展开态。与 [s40] 同语义。 */
    val s42 = 42.dp

    /** 44：浮窗按钮最小宽度、开关轨道、设置项最小宽度。 */
    val s44 = 44.dp

    /** 48：列表行的最小高度，同时满足无障碍点击目标下限。 */
    val s48 = 48.dp

    /** 52：设置行的最小高度。与 [s48] 同语义（改造前另一文件取 48）。 */
    val s52 = 52.dp
...
object StaMotion {
    ...
    /**
     * 循环/呼吸动画的周期。单独一组，不混进上面的过渡刻度：
     * 过渡描述「一次状态变化多久被感知到」，呼吸描述「静止内容多久起伏一次」；
     * 塞进同一条刻度只会逼其中一方迁就另一方。数值就是一直在用的原值。
     */
    const val pulse = 820

    /** 三点等待指示里单个点的呼吸周期（点与点之间错开 150ms）。 */
    const val dots = 600
}
