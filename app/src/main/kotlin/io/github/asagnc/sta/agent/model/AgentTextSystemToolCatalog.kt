    ...
    fun appendTo(tools: JSONArray) {
        tools
            ...
            .put(
                AgentToolSchema.function(
                    name = "submit_plan",
                    description = "把「打算怎么做」写成一份给用户看的方案。复杂任务（多文件改动、" +
                        "需求有歧义、要动系统设置、方案有多种走法）用它：先派子智能体检索、把细节" +
                        "搞清楚，再把方案写清楚提交。提交只是记录，不会拦下工具、也不会结束本轮——" +
                        "用户可能点「按此执行」（用 steps 建清单），也可能直接给新指令；不要当成" +
                        "已批准。方案与任务清单分工不同：方案回答「做什么、为什么」（一次产出、" +
                        "固定），清单回答「做到哪」（每轮更新）。digest 是给后续轮次注入的方向摘要，" +
                        "必须精简；content 是给人看的完整正文，可以详细，支持 markdown。",
                    parameters = JSONObject()
                        .put("type", "object")
                        .put(
                            "properties",
