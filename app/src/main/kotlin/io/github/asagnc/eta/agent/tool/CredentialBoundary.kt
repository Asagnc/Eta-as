 ...
 * 凭据路径的读取边界。
 *
 * provider API key 明文存在 App 私有的 `databases/eta.db` 里，而 Agent 同时拥有 Root 与网络工具：
...
internal object CredentialBoundary {
    ...
    private val DENIED_FILE_NAMES = listOf("eta.db", "eta.db-wal", "eta.db-shm", "eta.db-journal")
    ...
    fun deniesLoosely(dataDir: String, rawPath: String): Boolean =
        denies(dataDir, rawPath) || rawPath.contains("..") && rawPath.contains("eta.db")
